use crate::{
    data_handling::{TrainingDataset, mnist::load_mnist},
    error::{PredictiveCodingError, Result},
    model::{PredictiveCodingModel, create_from_config, load_model_snapshot},
};
...
pub fn save_training_config(config: &TrainConfig, output_path: &str) -> Result<()> {
    let file = std::fs::File::create(output_path).map_err(|source| {
        PredictiveCodingError::io("create training config", output_path, source)
    })?;
