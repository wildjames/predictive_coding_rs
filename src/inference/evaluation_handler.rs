...
use crate::{
    ...
    training::{load_dataset, load_training_config, validate_model_and_dataset_shapes},
};

use super::{InferenceModelHandler, InferencePrediction, read_label};
...
impl EvaluationRun {
    ...
    pub fn evaluate_and_write_results(&mut self) -> Result<EvaluationArtifacts> {
        ...
        let output_path =
            save_evaluation_results(&self.model_file, self.handler.model(), &summary)?;
...
pub fn save_evaluation_results(
        ...
        && !output_dir.as_os_str().is_empty()
    {
        std::fs::create_dir_all(output_dir).map_err(|source| {
            PredictiveCodingError::io("create evaluation artifact directory", output_dir, source)
        })?;
    }

    let output_file = std::fs::File::create(&output_path).map_err(|source| {
        PredictiveCodingError::io("create evaluation results", &output_path, source)
    })?;
...
{
    if dataset_size == 0 {
        return Err(PredictiveCodingError::invalid_data(
...
mod tests {
    use crate::{
        error::PredictiveCodingError,
        inference::{
            evaluation_handler::{
                EvaluationSummary, PreparedSample, evaluation_results_path,
                summarise_evaluation_samples,
            },
            inference_model_handler::{InferenceModelHandler, read_prediction},
        },
        test_utils::{DummyTrainingDataset, TempDir, tiny_relu_model},
    };
    use ndarray::array;
    ...
    fn summarise_evaluation_samples_rejects_empty_datasets() {
        let result: Result<EvaluationSummary, crate::error::PredictiveCodingError> =
            summarise_evaluation_samples(0, |_| Ok((0, 0, 0.0, 0.0)));
    ...
    fn summarise_evaluation_samples_returns_zero_confidence_when_all_predictions_are_wrong() {
        let summary: EvaluationSummary =
            summarise_evaluation_samples(2, |index| Ok((index, index + 1, 0.9, 3.0))).unwrap();
    ...
    fn evaluation_results_path_uses_model_directory() {
        ...
        assert_eq!(
            output_path,
            temp_dir.join("artifacts/evaluation_results.json")
        );
    }
}
