...
pub struct MnistDataset {
  ...
  pub images: Vec<Vec<u8>>, // binary pixel data, images are black and white
  pub labels: Vec<u8>, // Integers from 0 to 9
  pub image_width: u32,
...
pub fn load_mnist<P: AsRef<Path>>(images_path: P, labels_path: P) -> io::Result<MnistDataset> {
  ...
  for i in 0..num_images {
    ...
    images.push(images_idx.data[start..end].to_vec());

    // The labels are just one byte each, so we can read them directly
    labels.push(labels_idx.data[i]);
  }
...
pub fn output_image<P: AsRef<Path>>(data: &MnistDataset, index: usize, output_path: P) -> image::ImageResult<GrayImage> {
  let image_data = &data.images[index];
